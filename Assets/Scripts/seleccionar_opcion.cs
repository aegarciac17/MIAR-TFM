    ...
    public float ConvertirTextoADecimal(string texto)
    {
        ...
        if (partes.Length == 2)
        {
            if (float.TryParse(partes[0], out float numerador) && float.TryParse(partes[1], out float denominador))
            {
                ...
                //es un numero mixto que se ha colado
                string[] partes_numerador = partes[0].Split(' ');
                if (float.TryParse(partes_numerador[0], out float entero_mixto) && float.TryParse(partes_numerador[1], out float numerador_mixto) && float.TryParse(partes[1], out float denominador_mixto))
                {
                    // Convertir el numerador y el denominador a valores decimales y realizar la división
                    float valorDecimal = numerador+entero_mixto*denominador_mixto / denominador_mixto;
                    return valorDecimal;
                }
